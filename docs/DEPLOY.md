...
| `AGENT_COMMAND_SECRET_PREVIOUS` | Nothing rotates. Set it to the previous seed, with `AGENT_COMMAND_SECRET` holding the new one, and scheduled maintenance hands the new public half to each machine as it is seen online — signed by the old key, which is the only one they trust until it arrives. A machine that is asleep gets it on a later run, which is why this is a nightly sweep rather than a one-off. Unset it when every machine reports the new key |
