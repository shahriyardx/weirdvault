...
export function AuditBlindingDiagram({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <desc id="blind-desc">
        Your browser turns a hostname into an HMAC using a key derived from your password. The
        server stores only that opaque reference, so it can group events by host without ever
        learning the hostname.
      </desc>
