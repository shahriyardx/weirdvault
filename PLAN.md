...
| **No port forwarding, permanently** | Decided against, not blocked, and not deferred. `-L` means something on the user's machine listens on a TCP port. A tab cannot — there is no API, and the one that exists (Direct Sockets' `TCPServerSocket`) is Isolated Web Apps only, so this is not waiting on browser support. Delivering it would take a daemon on the *client* machine, which is the one thing the product promises you never install. That promise is worth more than the feature. A desktop app would make it easy and is not a reason to keep the door open here | None, and this row is the answer to anyone who proposes it. **This is not a ban on `direct-tcpip`.** That channel type is also how a bastion is traversed, and reaching a host through a jump box installs nothing on anybody's machine — see the ProxyJump row below. Rule out the *local listener*, not the primitive |
| **ProxyJump, not built yet** | Different feature, opposite constraints. `ProxyJump` opens a `direct-tcpip` channel on the bastion's session and runs a second, end-to-end handshake over it, so nothing is installed on the client, the bastion or the target, and the bastion sees a TCP connection rather than a session. `sshconfig.go` already parses it and the hosts page already tells importers it was dropped | Build it. It is the private-subnet answer that port forwarding is *not* needed for, and it composes with agents: one daemon per network instead of one per machine |
