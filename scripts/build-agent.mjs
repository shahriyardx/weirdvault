...
// Stamped into the binary so `webxterm-agent version` and the agent row in the
...
for (const [goos, goarch] of TARGETS) {
  const name = `webxterm-agent_${goos}_${goarch}`;
  execFileSync(
    "go",
...
const lines = readdirSync(OUT_DIR)
  .filter((f) => f.startsWith("webxterm-agent_"))
  .sort()
